...
[
  function(callback){
    let DestinationForListen = Destination;
    console.log('func1 :' + DestinationForListen);
  },
  function(DestinationForListen, callback)
  {
    var sqlForListen = "SELECT * FROM " + DestinationForListen + ";"
    pool.query(sqlForListen, function(err, rows){
      if(err){
        console.log('query_err :' + err);
        callback(err);
      }
      else{
        console.log('func2 :' + rows[0].P)
        let listen = rows[0].P
  ...
  function(listen, callback){
    console.log('func3 :' + listen)
    npkResponse.Listen_List_Output({listen}); //함수 결과를 output 파라미터에 저장
  }
];
...
class NPKRequest {
  ...
  actionRequest(npkResponse) {
    ...
    switch (actionName) {
      ...
      Destination = DestinationForListen.value

      //읽을 카테고리 데이터도 파라미터로 추가
      async.waterfall(Listen_List, function (err) {
